    ...
    def run_experiment(self):
        ...
        for Lambda in self.params['Lambda_range']:
            for r in range(R):
                ...
                print('ppm rel error: %f, num iter: %d' % (rel_error_z_over_2(x_est, x), num_iter))

                x_est, num_iter = pim_z_over_2(Y, x_init)
                err_pim = rel_error_z_over_2(x_est, x)
                num_iter_pim = num_iter
                print('pim rel error: %f, num iter: %d' % (rel_error_z_over_2(x_est, x), num_iter))

                x_est, num_iter = amp_z_over_2(Y, x_init, x_init2, Lambda)
                err_amp = rel_error_z_over_2(x_est, x)
                num_iter_amp = num_iter

                print('amp rel error: %f, num iter: %d' % (rel_error_z_over_2(x_est, x), num_iter))
                df = pd.concat([df,
...
if __name__ == '__main__':
    CompareZOver2Experiment(params={'N': 2000, 'R': 1, 'Lambda_range': np.arange(1.2, 3.2, 0.2), 'seed': 1})
