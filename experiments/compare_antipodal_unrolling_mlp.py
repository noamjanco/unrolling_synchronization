...
from models.unrolling_synchronization_antipodal_mlp import BuildModelMLP, TrainModelMLP, EvaluateModelMLP, loss_z_over_2
...
def task(N, R, Lambda, DEPTH, seed, epochs, L):
    ...
    model = BuildModelMLP(N,Lambda,DEPTH)
    ...
    TrainModelMLP(model, Y, x, x_init,x_init2,Y_val, x_val, x_val_init,x_val_init2, epochs)

    # Generate test data
    x, Y = generate_training_data_antipodal(N,Lambda,R, L)
    x_init = 1e-1*np.expand_dims(np.random.rand(R,N),axis=-1)
    x_init2 = 1e-1*np.expand_dims(np.random.rand(R,N),axis=-1)
    x_est, loss_nn = EvaluateModelMLP(model, Y, x, x_init, x_init2)
...
class CompareAntipodalUnrollingMLPExperiment(Experiment):
...
if __name__ == '__main__':
    ...
    # CompareAntipodalUnrollingMLPExperiment(params={'N': 20, 'R': 100, 'num_trials': 1, 'depth_range': [3], 'epochs': 10, 'Lambda': 0.2, 'L': 21})
    CompareAntipodalUnrollingMLPExperiment(params={'N': 20, 'R': 10000, 'num_trials': 1, 'depth_range': [3], 'epochs': 300, 'Lambda': 0.2, 'L': 21})
    # CompareAntipodalUnrollingExperiment(params={'N': 20, 'R': 10000, 'num_trials': 1, 'depth_range': [1, 3, 5, 9, 15, 20, 50], 'epochs': 300, 'Lambda': 0.2, 'L': 21})
