import numpy as np
import pandas as pd
from data_generation.generate_data import generate_training_data_z_over_2_bsc
from experiments.base_experiment import Experiment
from models.unrolling_synchronization_z_over_2_bsc import BuildModel, TrainModel, EvaluateModel, loss_z_over_2
...
def task(N, R, Lambda, DEPTH, seed, epochs):
    ...
    np.random.seed(seed)
    x, Y = generate_training_data_z_over_2_bsc(N,Lambda,R)
    ...
    x_val, Y_val  = generate_training_data_z_over_2_bsc(N, Lambda, R)
...
class CompareZOver2BSCUnrollingExperiment(Experiment):
...
if __name__ == '__main__':
    CompareZOver2BSCUnrollingExperiment(params={'N': 20, 'R': 10000, 'num_trials': 1, 'depth_range': [1, 3, 5, 9, 15, 20, 50], 'epochs': 1000, 'Lambda': 1.25})
