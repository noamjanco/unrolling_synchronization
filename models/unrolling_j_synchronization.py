...
class JConfigurationErrorBlock(keras.layers.Layer):
    def __init__(self, N, batchsize, global_indices):
        ...
        # self.alpha = tf.Variable(10*tf.ones(1), trainable=True)
        ...
        stddev = 1e-1
        n_hidden_layer = 256
        self.hidden_11 = Dense(n_hidden_layer, activation='relu', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
        self.hidden_12 = Dense(1, activation='tanh', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
        self.hidden_21 = Dense(n_hidden_layer, activation='relu', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
        self.hidden_22 = Dense(1, activation='tanh', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
        self.hidden_31 = Dense(n_hidden_layer, activation='relu', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
        self.hidden_32 = Dense(1, activation='tanh', kernel_initializer=tf.keras.initializers.RandomNormal(stddev=stddev))
    ...
    def call(self, H):
        ...
        j_comb = tf.reshape(j_comb, (j_comb.shape[0],-1))
        # j_comb = self.bn1(j_comb)

        s1 = self.hidden_11(j_comb)
        # s1 = self.bn1(s1)
        s1 = self.hidden_12(s1)
        s2 = self.hidden_21(j_comb)
        # s2 = self.bn1(s2)
        s2 = self.hidden_22(s2)
        s3 = self.hidden_31(j_comb)
        # s3 = self.bn1(s3)
        s3 = self.hidden_32(s3)

        s1 = tf.reshape(s1, (self.batchsize, -1))
        s2 = tf.reshape(s2, (self.batchsize, -1))
        s3 = tf.reshape(s3, (self.batchsize, -1))
